...
class UzbotAddFormNewsListener implements IParameterizedEventListener {
	...
	protected function readFormParameters() {
		if (isset($_REQUEST['newsCategoryIDs']) && is_array($_REQUEST['newsCategoryIDs'])) $this->newsCategoryIDs = ArrayUtil::toIntegerArray($_REQUEST['newsCategoryIDs']);
		
		$this->newsBigVersion = $this->newsIsDisabled = $this->newsOnlySidebar = $this->newsIsHot = 0;
		if (isset($_POST['newsIsDisabled'])) $this->newsIsDisabled = intval($_POST['newsIsDisabled']);
		if (isset($_POST['newsBigVersion'])) $this->newsBigVersion = intval($_POST['newsBigVersion']);
		if (isset($_POST['newsIsHot'])) $this->newsIsHot = intval($_POST['newsIsHot']);
		if (isset($_POST['newsOnlySidebar'])) $this->newsOnlySidebar = intval($_POST['newsOnlySidebar']);
		
		if (isset($_POST['newsSource'])) $this->newsSource = StringUtil::trim($_POST['newsSource']);
		if (isset($_POST['newsSourceUrl'])) $this->newsSourceUrl = StringUtil::trim($_POST['newsSourceUrl']);
		if (isset($_POST['newsImageID'])) $this->newsImageID = intval($_POST['newsImageID']);
	}
	...
	protected function validate() {
		...
		if ($notify->notifyTitle == 'news') {
			...
			foreach ($this->newsCategoryIDs as $categoryID) {
				...
				if ($category === null) throw new UserInputException('newsCategoryIDs', 'invalid');
			}
